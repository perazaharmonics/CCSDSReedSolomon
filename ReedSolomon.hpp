 ...
 * *   support for both encoding and decoding operations. , t=16 over GF(256) with field
 * *   generator polynomial: F(x)= x^8 +x^7 +x^2 + x + 1 (0x187, CCSDS 131.0-B-5 sec 4.3.3). 
 * *   Primitive element alpha^11 (CCSDS sec 4.3.4).
 ...
 * *
 * * Author:
 * *  JEP, J. Enrique Peraza, October 24, 2025
 * *
 * *
 * * References:
    ...
    class ReedSolomon
    {
        ...
        struct CodecState
        {
          ...
          {                             // ~~~~~~~~~ Encode ~~~~~~~~~~ //
            ...
            for (int i=0;i<K;++i)       // For each data byte
            {
              ...
              {                         // Yes, update parity
                ...
                //Deb("RS Encode: Data byte %d, feedback=0x%02X",i,fb);
              ...
              //Deb("RS Encode: Parity after data byte %d:",i);
          ...
          inline RSStatus Decode (
            ...
            int ner=0,                  // Number of erasures  
            bool dbase=false) const     // True if codeword symbols are in dual basis (CCSDS Blue book 131.0-B-5 sec 4.3 / Annex F)
          {                             // ~~~~~~~~~ Decode ~~~~~~~~~ //
            ...
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            // CCSDS 131.0-B-5 sec 4.3 / Annex F: channel R-S symbols are in the
            // dual (Berlekamp) basis. Convert the entire received codeword
            // (data + parity) dual -> conventional via tcb before syndrome
            // computation; the recovered dataword is then conventional basis
            // which is the Transer Frame consumers expect. Mirrors the tcb
            // conversion in Encode (dbase=true)
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            if (dbase==true)            // Codeword in dual basis?
            {                           // Yes,
              for (int i=0;i<N;++i)     // For each of the 255 symbols
                block[i]=T::tcb[block[i]]; // Convert dual -> conventional
            }                           // Done transforming to conventional
            ...
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            if (!VerifyCorrections(block)) // Were corrections successful?
            {                           // No, indicate failure
              sto.ok=false;             // Decode failed.
              sto.corr=-1;              // Indicate failure with -1
              return sto;               // Early return.
        ...
        protected:
          ...
          {                             // ~~~~~~~~~ ComputeSyndromes ~~~~~~~~~ //
            ...
            {                           // and for...
              ...
              {                         // Compute syndrome j
                ...
                //Deb("RS Syn: After byte %d, syndrome %d=0x%02X",i,j,cs.s[j]);
            ...
            {                           // Check each syndrome
              ...
              //Deb("RS Syn: Syndrome %d=0x%02X (index form)",i,cs.s[i]);
          ...
          {                             // ~~~~~~~~~ BerlekampMassey ~~~~~~~~~ //
            ...
            //Log("RS BM: Begin Berlekamp-Massey algorithm");
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            ...
            {                           // Yes, fold into Lambda
              ...
              {                         // Compute next erasure
                ...
                //Log("RS BM: After erasure %d at pos %d, lambda=",i,cs.eras[i]);
                //for (int k=0;k<=np;++k)
                //  Log("%02X ",cs.lambda[k]);
            ...
            {                           // Compute discrepancy in polynomial form
              ...
              {                         // Update T(x)
                ...
                {                       // Yes, update B(x)
                  ...
                  //Log("RS BM: Significant failure at r=%d, updated B(x)",r);
                }                       // Done updating B(x)
                ...
                {                       // Shift T(x) into B(x)
                  ...
                  std::memmove(&cs.b[1],&cs.b[0],np); // Shift B(x) coeffs
                  cs.b[0]=T::a0;        // Set B_0=0
                  //Log("RS BM: Non-significant failure at r=%d, shifted B(x)",r);
                }                       // End else non-significant failure
                //std::memcpy(cs.lambda,cs.t,sizeof(cs.lambda));// Copy T(x) to Lambda(x)
                std::copy_n(cs.t,np+1,cs.lambda); // Copy T(x) to Lambda(x)
              }                         // End else non-zero discrepancy
              //Log("RS BM: r=%d lambda=",r);
              //for (int i=0;i<=np;++i)
              //  Log("%02X ",cs.lambda[i]);
            }                           // End while r<=np
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            ...
            {                           // Convert to index form
              ...
              //Log("RS BM: Final lambda[%d]=%02X",i,cs.lambda[i]);
            }                           // Done converting lambda coeffs to index form
            //Log("RS BM: Completed Berlekamp-Massey algorithm (success)");
          ...
          {                             // ~~~~~~~~~~ ChienSearch ~~~~~~~~~~ //
            ...
            for (int i=1,k=T::iprim-1;i<=T::N;++i,k=(k+T::iprim)%T::N)
            {
              ...
              {                         // Evaluate lambda at alpha^k
                ...
                //Log("RS CS: i=%d j=%d reg=%02X q=%02X",i,j,cs.reg[j],q);
              }                         // End for each lambda coeff
              ...
              //Log("RS CS: Found root %d at position %d",cs.nr,cs.root[cs.nr]);
              if (++cs.nr==cs.ordl)     // Found all roots?
                break;                  // Yes, exit search 
            }                           // End for each element in field
            //Log("RS CS: Completed Chien's Search algorithm, found %d roots",cs.nr);
          ...
          {                             // ~~~~~~~~~ ComputeOmega ~~~~~~~~~ //
            ...
            //Log("RS CO: Begin Omega polynomial computation");
            for (int i=0;i<np;++i)      // For each coeff up to # of parity symbols
            {                           // Compute omega coeff i
              ...
              {                         // Compute term j
                ...
                {                       // Yes, update temp
                  ...
                  //Log("RS CO: Omega update i=%d j=%d s=%02X lambda=%02X tmp=%02X",
              ...
              //Log("RS CO: Omega[%d]=%02X",i,cs.om[i]);
            }                           // End for each coeff
            cs.om[np]=T::a0;            // Clear highest omega coeff
            //Log("RS CO: Completed Omega polynomial computation");
          }                             // ~~~~~~~~~ ComputeOmega ~~~~~~~~~ //
          // ~~~~~~~~~~~~~~~~~~~~~~~~~~ //
          ...
          {                             // ~~~~~~~~ ForneyCorrection ~~~~~~~~ //
            ...
            //Log("RS FC: Begin Forney's error correction");
            ...
            {                           // Compute error magnite
              ...
              {                         // Compute term i
                ...
                {                       // Yes, update num1
                  ...
                  //Log("RS FC: num1 update j=%d i=%d om=%02X num1=%02X",j,i,cs.om[i],num1);
                }                       // End if non-zero coeff   
              ...
              //Log("RS FC: num2 for root %d = %02X",j,num2);
              // ~~~~~~~~~~~~~~~~~~~~~~ //
              ...
              {                         // Compute denominator 
                ...
                {                       // Yes, update den
                  ...
                  //Log("RS FC: den update j=%d i=%d lambda=%02X den=%02X",j,i,cs.lambda[i+1],den);
                }                       // End if non-zero coeff
              ...
              {                         // Yes, that's a Forney failure.
                ...
                //Err("RS FC: Forney correction failure at position %d (denominator zero)",cs.loc[j]);
              ...
              {                         // No, compute error magnitude
               ...
               //Log("RS FC: Corrected error at position %d, magnitude %02X",
               //    cs.loc[j],T::ato[(T::idxof[num1]+T::idxof[num2]+T::N-T::idxof[den])%T::N]);               
              }                         // End if numerator non-zero
            }                           // End for each root
            //Log("RS FC: Completed Forney's error correction");
          }                             // ~~~~~~~~ ForneyCorrection ~~~~~~~~ //
          // ~~~~~~~~~~~~~~~~~~~~~~~~~~ //
          ...
          {                             // ~~~~~~~~ VerifyCorrections ~~~~~~~~ //
            ...
            //Log("RS VC: Begin verification of corrections");
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            // Recompute syndromes
            // ~~~~~~~~~~~~~~~~~~~~~~~~ //
            int synerr{0};
            ...
            for (int j=1;j<N;++j)       // For the next bytes in codeword
            {                           // and for...
              for (int i=0;i<np;++i)    // The number of roots (parity symbols)
              {                         // Compute syndrome i
                if (s2[i]==0)           // If current syndrome is zero
                  s2[i]=block[j];       // Next byte is the new syndrome
                else                    // Else, non zero, compute syndrome
            ...
            for (int i=0;i<np;++i)      // For the number of parity symbols (roots)
            {                           // Check each syndrome
              synerr|=s2[i];            // Convert to index form
              //Log("RS VC: Recomputed syndrome[%d]=%02X",i,s2[i]);
            }                           // End for each parity symbol
